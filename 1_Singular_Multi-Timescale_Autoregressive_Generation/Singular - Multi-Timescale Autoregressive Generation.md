...
* **Gated Dual-Rate (Singular-SSM)**: Validation PPL = **821.53**, operating at a highly stable average wake frequency of **1.44 Hz** (2.9% $T_1$ active steps) and achieving a **3.82× GFLOPS computational compression ratio**. Physical hardware profiling on the NVIDIA L20 GPU demonstrates a **7.08× physical wall-clock speedup** (2.85 ms/token vs. 20.17 ms/token) and **88.4% physical GPU energy savings** (225.7 mJ/token vs. 1948.3 mJ/token).
