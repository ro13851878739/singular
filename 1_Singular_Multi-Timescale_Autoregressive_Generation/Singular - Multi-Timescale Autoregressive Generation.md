...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Monolithic Baseline (Mamba-370M)** | 100.0% (50.0 Hz) | 31.20 | 1.0× (Dense baseline) | 1.00× (20.26 ms/token) | Baseline (2076.6 mJ) | Sec. VI.E |
...
| **Gated Dual-Rate (Singular-SSM)** | **2.9% (1.44 Hz)** | **821.53** | **3.82× (GFLOPS model)** | **4.41× (4.60 ms/token)** | **76.6% savings (486.5 mJ)** | Sec. VI.E |
...
* **Gated Dual-Rate (Singular-SSM)**: Validation PPL = **821.53**, operating at a highly stable average wake frequency of **1.44 Hz** (2.9% $T_1$ active steps) and achieving a **3.82× GFLOPS computational compression ratio**. Real-time hardware profiling on the NVIDIA L20 GPU under active quality-paired waker activation ($\Gamma_0 = 0.400$, 28.6% active wake rate) demonstrates a **4.41× physical wall-clock speedup** (4.60 ms/token vs. 20.26 ms/token monolithic baseline) and **76.6% physical GPU energy savings** (486.5 mJ/token vs. 2076.6 mJ/token monolithic baseline), showing that the dual-rate architecture retains physical efficiency while actively recovering representation quality.
